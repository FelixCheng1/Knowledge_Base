...
# 全局MinIO客户端实例（单例模式，懒加载，避免重复创建连接，提升性能）
_minio_client = None
...
def _create_minio_client() -> Minio:
    ...
    return Minio(
        ...
        secure=minio_config.minio_secure       # 是否启用HTTPS（True/False），取自配置避免硬编码
    )
...
def _create_bucket_ready(client: Minio):
    ...
    说明：每次调用都重新设置访问策略（幂等操作），避免桶已存在但策略缺失/被改动的问题
