...
def step_2_prepare_collection(milvus_client:MilvusClient):
    ...
    if not milvus_client.has_collection(collection_name=milvus_config.chunks_collection):
      ...
      schema = milvus_client.create_schema(
          auto_id = True,   # 主键自增长
          enable_dynamic_filed = True # 动态字段
      )
      # 3.2 创建约束字段
...
def step_3_delete_old_data(milvus_client:MilvusClient, item_name:str, file_title:str):
    """
    删除旧数据 根据item_name删除
    Args:
      milvus_client:
    ...
    filter_expr = (
      f'item_name == "{escape_milvus_string(item_name)}" and '
      f'file_title == "{escape_milvus_string(file_title)}"'
    )
...
def step_4_insert_collections(milvus_client:MilvusClient,chunks:list):
    """
    插入集合的数据！
    Args:
      milvus_client:
...
def node_import_milvus(state: ImportGraphState) -> ImportGraphState:
    ...
    # 3. 删除旧数据
    step_3_delete_old_data(milvus_client, state['item_name'], state['file_title'])
    # 4. 插入chunks的数据即可
