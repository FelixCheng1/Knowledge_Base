...
def step_1_check_answer(state:QueryGraphState):
    ...
    if answer:
        ...
        if is_stream:
            ...
            # 整包作为一次 delta 推送，保证前端流式界面能立即展示该答案（反问/拒绝场景）
...
def step_2_construct_prompt(state: QueryGraphState) -> str:
    ...
    """   
    将reranked的数据构造 "元数据头 + 正文" 格式的字符串，例如：
    ...
    used = 0
    # 对reranked_docs进行遍历
    for num, doc in enumerate(reranked_docs, start=1):
    ...
    if history:
        for msg in history:
         # MongoDB存储格式为 {"role": "user"/"assistant", "text": "..."}
         role = msg.get("role")
         text = msg.get("text")
         history_text = ""
...
def _extract_images_from_docs(docs):
    ...
    for i, doc in enumerate(docs):
        ...
        # 2. 检查 text 字段中的MD图片（主要针对 Loacl Chunks）
        text = (doc.get("text") or "").strip()
        if text:
            matches:list[str] = md_img_pattern.findall(text)
...
def node_answer_output(state:QueryGraphState):
    ...
    # 阶段一：检查answer是否存在，如果存在直接输出answer中的内容
    answer_exists = step_1_check_answer(state)

    # 阶段二：如果没有answer，则构建Prompt
    if not answer_exists:
        prompt= step_2_construct_prompt(state)
        state["prompt"] = prompt

        # 阶段三：如果没有answer则调用大模型输出答案
        step_3_generate_response(state, prompt)

    # 提取图片URL（用于历史记录和前端展示）
    image_urls = _extract_images_from_docs(state.get("reranked_docs") or [])

    # 阶段四：把答案写进mongodb的history里
    # 注意：仅当本节点新生成答案（answer_exists=False）时才写历史；
    # 反问/拒绝场景的答案已在 node_item_name_confirm.step_7_write_history 写入，避免重复
    if state.get("answer") and not answer_exists:
