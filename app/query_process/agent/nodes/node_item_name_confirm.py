...
def step_3_extract_info(original_query, history) -> Dict:
    """
    利用LLM从当前问题以及历史会话中提取出主要询问的商品名称item_names（可多个，JSON列表形式）
    若商品名不够明确则返回空列表，同时根据上下文重新改写问题，保证问题独立完整

    :param original_query: 字符串 - 用户当前原始查询问题（如："这个多少钱？"）
...
def step_4_vectorize_and_query(item_names:list) -> List[dict]:
    """
    ...
    """
    logger.info(f"Step 4: 开始对商品名进行向量化和查询: {item_names}")
    ...
    if not collection_name:
        logger.error("在.env中未找到collection_name")
        return results

    #  对所有商品名称批量生成BGEM3向量（dance + sparse）， 相比逐个生成提高处理效率
    ...
    for i in range(len(item_names)):
        try:
            ...
            # 构造的Milvus混合搜索请求对象，传入稠密/稀疏 向量，指定返回Top5匹配结果
            ...
            # 默认配置：稠密/稀疏 向量权重 0.8/0.2 ，开启评分归一化
            search_res = hybrid_search(
                client=client,          # Mlivus客户端连接实例
                collection_name=collection_name,    # 目标向量集合名
                reqs=reqs,              # 混合搜索请求对象列表
                ranker_weights=(0.8, 0.2),  # 稠密/稀疏 向量评分权重配比
                limit=5,                # 最终返回Top5匹配的结果
                norm_score=True,        # 开启评分归一化
            ...
            # 初始化当前商品名的匹配结果列表，存储匹配到的商品名+对应的相似度评分
            matches = []
            #   [
            #     [
            #         {
                    ...
                    matches.append(
                        {
                            ...
                            "score":hit.get("distance") # 0-1相似度打分
                        }
                    )
            results.append({
...
def step_5_align_item_names(query_results:List[Dict[str,Any]]) -> dict:
    """
    ...
    confirmed_item_name:List[str] = []
    # 初始化候选商品名列表（低置信度，需用户确认的商品名）
    options:List[str] = []

    logger.info(f"获取到的待处理数据源：{query_results}")
    for res in query_results:
        ...
        # 规则a：只有一个高置信度结果:评分 > 0.85 -> 直接确认该商品名
        if len(high) == 1:
            confirmed_item_name.append(high[0].get("item_name"))
        ...
        if len(high) > 1:
            ...
            # 将选中的结果加入确认商品名列表
            confirmed_item_name.append(picked.get("item_name"))
    ...
    return {
        "confirmed_item_name":list(set(confirmed_item_name)),
        "options":list(set(options))
    }

@step_log("step_6_check_confirmation")
def step_6_check_confirmation(state:QueryGraphState, align_result, session_id, history, rewritten_query):
    """
    ...
    if confirmed:
        ...
        for msg in history:
            if not msg.get("item_names"):   # 仅更新item_names为空的历史消息
                ...
                if mid:
                    ids_to_update.append(str(mid))  # 转换为字符串，避免ID格式问题

        # 若存在需要更新的消息ID，批量更新历史消息的消息名关联
        if ids_to_update:
    ...
    state["answer"] = f"抱歉，未找到相关产品，请提供准确型号以便我为您查询。"
    state["item_names"] = []
    return state

@step_log("step_7_write_history")
def step_7_write_history(state:QueryGraphState, session_id, history, rewritten_query, message_id):
    """
...
def node_item_name_confirm(state: QueryGraphState) -> QueryGraphState:
    ...
    # 记录任务开始
    add_running_task(state["session_id"], "node_item_name_confirm", is_stream)
    ...
    # 更新 State 中的 rewrite—query
    state["rewritten_query"] = rewritten_query

    # 创建对齐后的结果字典
    align_result = {}

    # 4. & 5. 如果有提取到的商品名，进行搜索和对齐
    if len(item_names) > 0:
