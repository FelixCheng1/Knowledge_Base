...
# 断崖阈值（绝对）
RERANK_GAP_ABS: float = 1.5

@step_log("step_1_merge_docs")
def step_1_merge_docs(state:QueryGraphState) -> List[Dict]:
    """
    阶段一：文档合并与标准化
    ...
    """
    # 1. 提取输入源
    rrf_docs:List[Dict] = state.get("rrf_chunks") or []
    web_docs:List[dict] = state.get("web_search_docs") or []
    ...
    for i , doc in enumerate(rrf_docs):
       # 这里的 doc 应当已经是字典（由上游 _as_entity_list 保证）
    ...
    for i, doc in enumerate(web_docs):
       # 兼容不同字段名：优先取 snippet (摘要)，其次 content
        text = (doc.get("Content") or doc.get("snippet") or "").strip()
...
def step_2_rerank_docs(state:QueryGraphState, doc_items:List[Dict]):
    """
    阶段二：对文档进行重排序
    - 输入 doc_items：[{ text,doc_id}, ...]（由第一阶段产出）
    - 输出：在 state 中写入 reranked_docs（结构化列表）
    """
    # 获取状态中的rewritten_query 或者 original_query
...
def step_3_topk(scored_docs:List[Dict]) -> List[Dict]:
    ...
    """
    # 硬上限：最多取前10条，取全局变量与实际文档数的较小值(避免索引越界)
    # 注：max—topk从全局变量读取，不依赖外埠状态，保证逻辑一致性
    max_topk = min(RERANK_MAX_TOPK, len(scored_docs))
    min_topk = RERANK_MIN_TOPK      # 硬下限：至少保留的文档数量
    gap_ratio = RERANK_GAP_RATIO    # 相对断崖阈值：分数下降的相对比例阈值
    gap_abs = RERANK_GAP_ABS        # 绝对断崖阈值：分数下降的绝对值阈值

    # 断崖截断的核心逻辑：从min-tok之后开始检测分数断崖，出现则提前阶段
    topk = max_topk # 默认值：无断崖时取满硬上限
    if topk > min_topk:
        for i in range(min_topk - 1, max_topk - 1):
            s1 = scored_docs[i].get("score")    #当前文档的分数 
            s2 = scored_docs[i + 1].get("score")#下一个位置文档的分数

            gap = s1 - s2   # 计算相邻文档间分数的绝对差值（已降序 gap >= 0）
            # 计算相对差距：绝对差距 / 当前文档分数 (+ 1e-6 避免除数为0/极小值，防止程序报错)
            val = gap / (abs(s1) + 1e-6)
            # 触发断崖式截断条件：绝对差距 ≥ 绝对阈值 or 相对阈值
            # 满足任一条件，说明下一条文档的相关性骤降，截断到当前位置
            if gap > gap_abs or val > gap_ratio:
                logger.info(f"Step 3: 触发断崖截断 @ index={i} (Score {s1:.4f} -> {s2:.4f}, Gap={gap:.4f})")
                topk = i + 1    # 最终取 i + 1 条（索引转实际位置）
                break   # 触发截断后立即退出循环，不再检测后续位置

    # 按最终计算的topk的值，截取前topk条文档
