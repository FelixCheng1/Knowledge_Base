...
def node_search_embedding(state:QueryGraphState):
    ...
    query = state.get("rewritten_query")    # 提取改写后的用户问题
    item_names = state.get("item_names")    # 提取已确认的标准化商品名列表
...
if __name__ == "__main__":
    ...
    test_state = {
        ...
        "rewritten_query": "RS PRO RS-12数字万用表使用说明",  # 模拟改写后的查询
        "item_names": ["RS PRO RS-12数字万用表"],  # 模拟已确认的商品名
        "is_stream": False
    }
